...
% 上下両方の長さを計算
upperHeight = centerY - U_start + 1;
lowerHeight = D_end - centerY;

% 短い方の領域を選ぶ
commonHeight = min(upperHeight, lowerHeight);

% 上下方向の解析領域を設定
U_end = U_start + commonHeight - 1;
D_start = D_end - commonHeight + 1;

% 領域チェック
if U_start < 1 || D_end > frameHeight
    error('指定されたY座標に基づく上下の領域が画像の範囲を超えています。');
end
...
imagesc(maxAmplitude); axis image; colorbar;
hold on;
plot([centerX, centerX], ylim, 'r--', 'LineWidth', 1.5); % 左右対称線
plot(xlim, [centerY, centerY], 'g--', 'LineWidth', 1.5); % 上下対称線
...
% 三段目: 上下対称性解析結果（振幅、位相、周波数）
subplot(3, 3, 7);
imagesc(ampDiffUD); axis image; colorbar;
title('Amplitude Symmetry (U-D)');
xlabel('Pixels (x)'); ylabel('Pixels (y)');

subplot(3, 3, 8);
imagesc(phaseDiffUD, [0, 2*pi]); axis image; colorbar;
title('Phase Symmetry (U-D)');
xlabel('Pixels (x)'); ylabel('Pixels (y)');
colorbar('Ticks', [0, pi, 2*pi], 'TickLabels', {'0', '\pi', '2\pi'}); 

subplot(3, 3, 9);
imagesc(freqDiffUD, [0, max(freqDiffUD(:))]); axis image; colorbar;
title('Frequency Symmetry (U-D)');
xlabel('Pixels (x)'); ylabel('Pixels (y)');

% ウィンドウのサイズを指定
set(resultFig, 'Position', [100, 100, 1200, 900]);  % 横1200, 縦900

% 保存ダイアログを最前面に表示
figure(resultFig);  % 保存ダイアログを表示する前に最前面に移動
